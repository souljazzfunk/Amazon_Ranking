...
def fetch_data_with_requests(url):
    ...
    for retry_count in range(5):  # Maximum retries: 5
        ...
        return exit_with_error("Maximum retries reached for status code 503. Exiting.")
    ...
    if not ul_elems or len(ul_elems) < 2:
        return exit_with_error("Could not find ranking data.")
...
def take_screenshot_with_selenium(url, screenshot_links):
    ...
    options.add_argument('--window-size=2400,1080')
    ...
    img_binary = div_elm.screenshot_as_png
    img = Image.open(io.BytesIO(img_binary))
    img.save(f'amazon_ranking_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.png')
    ...
    for link in screenshot_links:
        ...
        img_binary = grandparent_elm.screenshot_as_png
        img = Image.open(io.BytesIO(img_binary))
        img.save(f'amazon_ranking_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.png')
